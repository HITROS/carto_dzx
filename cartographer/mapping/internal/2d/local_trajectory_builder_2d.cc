...
LocalTrajectoryBuilder2D::TransformToGravityAlignedFrameAndFilter(
  ...
  // 进行体素化滤波
  // 去除不合理的范围的点、合并相同位置的点等，从而减少点云数量，提高点云质量
...
// scan match
/*
  1.先用real time csm进行scan match
  2.把匹配好的位姿给到ceres scan进行优化，来最终得到位姿
  3.然后再用这个优化好的位姿来更新位姿差值器
*/
...
std::unique_ptr<transform::Rigid2d> LocalTrajectoryBuilder2D::ScanMatch(
  ...
  // realtimeCSM将会为ceres优化提供一个很好的初始位姿
...
// ceres优化函数，进行匹配
  ceres_scan_matcher_.Match(pose_prediction.translation(), 
                            initial_ceres_pose,// 初始位姿估计
                            filtered_gravity_aligned_point_cloud,// 对应的点云数据
                            *matching_submap->grid(), pose_observation.get(),// 构建子地图的概率栅格地图
                            &summary);
  // 统计残差
  if (pose_observation) 
  {
...
// 判断点云数据是否为空
  ...
  // 把点云数据通过估计出来的Pose,转化为在局部坐标系中的点云数据
