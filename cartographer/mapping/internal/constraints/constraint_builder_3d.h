...
// 混合任意数量的对“可能添加约束”、“可能添加全局约束”和“通知结束节点”的调用，然后调用“完成时”一次。完成所有计算后，将调用“回调”并得到结果
// 另一个可能是添加(全局)约束()/何时完成()循环
...
class ConstraintBuilder3D {
 public:
  ...
  // 子地图位姿和节点位姿都是在地图框架里都是对齐好的
  // The pointees of 'submap' and 'compressed_point_cloud' must stay valid until
  // all computations are finished.
  // 子地图数据和点云数据必须保持有效直到计算完成
 ...
 // 调用FAST CSM进行匹配
  ...
  // 返回的栅格和scan match结果必须要等到'creation_task_handle'完成后才能调用
