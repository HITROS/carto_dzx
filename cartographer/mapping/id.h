...
// 使用唯一轨迹ID和该轨迹内节点的从零开始的索引的组合来唯一的标识轨迹节点
struct NodeId 
{
...
// 使用唯一轨迹ID和该轨迹内子图的从零开始的索引的组合来唯一标识子图。
struct SubmapId 
{
  int trajectory_id;
  int submap_index;
 ...
 public:
  ...
  void Trim(const IdType& id) {
    ...
    if (std::next(it) == trajectory.data_.end()) {
      ...
      // 我们正在从该轨迹中删除索引最高的数据。
      // 我们假设我们将不再追加任何内容 
      // 如果这样做，我们将必须确保正确选择索引中的间隙以保持正确的连接性
      // 
      trajectory.can_append_ = false;
    }
