...
Node::Node(
  ...
  /*
    从后面对这些函数的定义可以看出，启动节点之后，之后所有的操作全部由map_builder_bridge来实现
*/ 
...
bool Node::HandleSubmapQuery(
  ...
  carto::common::MutexLocker lock(&mutex_);
  // 调用了map_builder_bridge,而这个函数定义在头文件里，定义了map_builder_bridge的接口
  // 接口实现在cartographer里map_builder.cc进行实现 
  map_builder_bridge_.HandleSubmapQuery(request, response);
  return true;
}
// 这里同样也是 调用了map_builder_bridge
