...
//对传感器消息进行封装，将代码转到cartographer里面 
// 构造函数
//  TrajectoryBuilderInterface型的一个指针变量。
// 继续跟踪代码可以发现，cartographer中各种消息都统一调用了这个成员类的虚函数AddSensorData(). 
// 而CollatedTrajectoryBuilder继承了这个类并实现了AddSensorData()函数。
