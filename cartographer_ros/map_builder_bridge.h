 ...
 public:
  struct TrajectoryState {
    ...
    struct LocalSlamData {
      ...
      ::cartographer::transform::Rigid3d local_pose;// 优化匹配之后的位姿 
      ::cartographer::sensor::RangeData range_data_in_local;// 激光数据 
    };
    std::shared_ptr<const LocalSlamData> local_slam_data;// 局部slam数据 
    cartographer::transform::Rigid3d local_to_map;// 这里是坐标变换关系 
    std::unique_ptr<cartographer::transform::Rigid3d> published_to_tracking;// 位姿插值器 
